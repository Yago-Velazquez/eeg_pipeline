"""
scripts/visit_badrate.py

Visit-level bad rate summary. Standalone EDA script.
"""
...
def main():
    ...
    print(f"\nGlobal bad rate: {df['is_bad'].mean():.4f}")
    print(
        f"Highest visit:   visit {summary.loc[summary['bad_rate'].idxmax(), 'visit']}"
        f"  bad_rate={summary['bad_rate'].max():.4f}"
    )
    print(
        f"Lowest visit:    visit {summary.loc[summary['bad_rate'].idxmin(), 'visit']}"
