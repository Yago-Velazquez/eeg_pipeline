...
def run_condition(
    ...
    for fold, (tr_idx, val_idx) in enumerate(gkf.split(X_df, y, groups)):
        ...
        xgb.fit(X_tr_np, y_tr, eval_set=[(X_val_np, y_val)], sample_weight=sw_tr, verbose=False)
...
def main():
    ...
    for cname, keep_cols in conditions:
        ...
        r = run_condition(X_df, y, groups, scale_pos_weight, keep_cols, cname, sample_weights)
        results.append(r)
        wandb.log({
