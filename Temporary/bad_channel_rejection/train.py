...
def load_prepared_data(bad_threshold: int = 2):
    ...
    sample_weights : np.ndarray  (18900,)  disagreement-based per-sample weights
    """
    X_raw, y, groups, feature_cols, scale_pos_weight, sample_weights = build_feature_matrix(
        DATA_PATH,
    ...
    return X, y, groups, scale_pos_weight, preprocessor, sample_weights
...
def run_cv(X: np.ndarray, y: np.ndarray, groups: np.ndarray,
           spw: float, bad_threshold: int,
           sample_weights: np.ndarray) -> tuple[list[dict], np.ndarray, np.ndarray]:
...
def train_with_wandb(bad_threshold: int = 2):
    ...
    # Load data
    X, y, groups, spw, preprocessor, sample_weights = load_prepared_data(bad_threshold)
    wandb.config.update({
    ...
    fold_metrics, oof_y_true, oof_y_prob = run_cv(X, y, groups, spw, bad_threshold, sample_weights)
    ...
    final_model, model_path = train_final_model(
        X, y, spw, best_n, bad_threshold, sample_weights
    )
